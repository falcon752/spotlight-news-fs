...
export default function Tables() {
  ...
  const handleClearPosts = async () => {
    if (!posts.length) return MySwal.fire("Nothing to clear", "", "info");
    const result = await MySwal.fire({
    ...
    if (result.isConfirmed) await clearPosts();
  };

  const handleClearVideos = async () => {
    if (!videos.length) return MySwal.fire("Nothing to clear", "", "info");
    const result = await MySwal.fire({
    ...
    if (result.isConfirmed) await clearVideos();
  };
    ...
    () => ({
      ...
      rows: filterPosts.map((post) => {
        ...
        return {
          ...
          image: post.img ? <PostImage src={post.img_url} alt={post.title} /> : null,
          date: <MDTypography variant="caption">{post.created_at ? new Date(post.created_at).toLocaleString() : "-"}</MDTypography>,
          actions: (
            <MDBox display="flex" justifyContent="center" gap={1}>
              <MDButton size="small" variant="gradient" color="info" onClick={() => navigate("/create-form", { state: { type: "post", data: post } })}>Edit</MDButton>
              <MDButton size="small" variant="gradient" color="error" onClick={() => handleDeletePost(post.id)}>Delete</MDButton>
            </MDBox>
          ),
        };
      }),
      ...
      rows: filterVideos.map((video) => {
        ...
        if (video.video_url) {
          try {
            ...
            videoId = url.hostname.includes("youtu.be") ? url.pathname.slice(1) : url.searchParams.get("v") || "";
          } catch { videoId = ""; }
        }
        ...
        return {
          ...
          thumbnail: thumbnailUrl ? <VideoThumbnail src={thumbnailUrl} alt={video.title} onClick={() => MySwal.fire({
          ...
          actions: (
            <MDBox display="flex" justifyContent="center" gap={1}>
              <MDButton size="small" variant="gradient" color="info" onClick={() => navigate("/create-form", { state: { type: "video", data: video } })}>Edit</MDButton>
              <MDButton size="small" variant="gradient" color="error" onClick={() => handleDeleteVideo(video.id)}>Delete</MDButton>
            </MDBox>
          ),
        };
      }),
        ...
        <Grid container spacing={6}>
            ...
            <Card>
              <MDBox mx={2} mt={-3} py={3} px={2} variant="gradient" bgColor="info" borderRadius="lg" coloredShadow="info" position="relative" display="flex" alignItems="center">
                ...
                <MDBox sx={{ position: "absolute", left: "50%", transform: "translateX(-50%)" }}>
                  <MDInput label="Search posts/videos" value={search} onChange={(e) => setSearch(e.target.value)} size="small" sx={{
                    width: 300, input: { color: "#fff" }, label: { color: "#fff" },
              ...
              <MDBox p={2} display="flex" justifyContent="flex-end">
                <MDButton variant="gradient" color="error" onClick={handleClearPosts}>Clear Table</MDButton>
              </MDBox>
            </Card>
          </Grid>
          ...
          <Grid item xs={12}>
            <Card>
              ...
              <MDBox p={2} display="flex" justifyContent="flex-end">
                <MDButton variant="gradient" color="error" onClick={handleClearVideos}>Clear Table</MDButton>
              </MDBox>
            </Card>
          </Grid>
        </Grid>
